...
import { useFlashUpdate, useUpdateFlashUpdate } from '@/hooks/use-flash-updates'
...
export default function EditFlashUpdatePage() {
  ...
  const { data: flashUpdate, isLoading } = useFlashUpdate(id)
  const updateFlashUpdate = useUpdateFlashUpdate()
  ...
  // Reset form when flash update loads
  useEffect(() => {
    if (flashUpdate) {
      reset({
        titleNe: flashUpdate.titleNe,
        titleEn: flashUpdate.titleEn,
        contentNe: flashUpdate.contentNe,
        contentEn: flashUpdate.contentEn,
        excerptNe: flashUpdate.excerptNe || '',
        excerptEn: flashUpdate.excerptEn || '',
        metaTitle: flashUpdate.metaTitle || '',
        metaDescription: flashUpdate.metaDescription || '',
        featuredImageId: flashUpdate.featuredImageId,
      })
      setFeaturedMedia(flashUpdate.featuredImage as Media | null)
    }
  }, [flashUpdate, reset])
  ...
  const onSubmit = async (data: FlashUpdateFormData) => {
    try {
      await updateFlashUpdate.mutateAsync({ id, data })
      toast.success('Flash update updated successfully')
      router.push('/admin/flash-updates')
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to update flash update'
  ...
  if (!flashUpdate) {
    return <div>Flash update not found</div>
  }

  return (
    <div className="space-y-6">
      <PageHeader
        title="Edit 24hrs Update"
        description="Edit flash update (auto-expires after 24 hours)"
        actions={
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              ...
              <Button type="submit" className="w-full" disabled={updateFlashUpdate.isPending}>
                <Save className="w-4 h-4 mr-2" />
                {updateFlashUpdate.isPending ? 'Updating...' : 'Update'}
              </Button>
            </CardContent>
          </Card>
