...
const storage = multer.diskStorage({
  destination: (_req, _file, cb) => {
    ...
    const uploadDirectory = path.join(uploadBaseDirectory, year, month)
...
async function handleGet(req: AuthedRequest, res: NextApiResponse<ApiResponse<unknown>>): Promise<void> {
...
async function handlePost(req: AuthedRequest, res: NextApiResponse<ApiResponse<unknown>>): Promise<void> {
  if (!hasRole(req.user.role, [Role.AUTHOR, Role.ADMIN, Role.SUPERADMIN])) {
    sendJson(res, 403, {
      success: false,
      data: null,
  ...
  if (uploadedFile.size > maxSize) {
    const relativePath = path
      .relative(publicRoot, uploadedFile.path)
      .replace(/\\/g, '/')
