  ...
  return (
    <>
      <button
        ...
        className="fixed z-40 right-3 top-32 md:top-36 flex items-center gap-2 px-3 py-2.5 bg-news-red text-white font-semibold rounded-full shadow-lg hover:shadow-xl hover:bg-news-red-dark transition-all duration-300"
        aria-label={isNepali ? 'फ्ल्यास न्युज' : 'Flash News'}
      >
        <Zap className="w-5 h-5 fill-current" />
        <span className="text-sm">
          {isNepali ? 'फ्ल्यास' : 'Flash'}
        </span>
      </button>
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <div className="flex items-center justify-between p-4 border-b border-news-border">
          <h2 className={cn('text-lg font-bold', isNepali ? 'font-nepali' : '')}>
            {isNepali ? 'फ्ल्यास न्युज' : 'Flash News'}
          </h2>
        ...
        <div className="overflow-y-auto h-[calc(100%-65px)] pb-20 dark:bg-news-bg-dark">
          {flashUpdates.length === 0 ? (
            <p className="p-4 text-center text-gray-500">
              {isNepali ? 'कुनै फ्ल्यास न्युज छैन' : 'No flash news available'}
            </p>
          ) : (
